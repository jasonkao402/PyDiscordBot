...
class Music(commands.Cog):
    ...
    async def _quiz(self, ctx, *args):
        ...
        await ctx.send("preview ans = "+str(set(args)))
        print("preview ans = ",set(args))
        await player.ans_que.put(set(args))
        await player.queue.put(source)
    ...
    async def _guess(self, ctx, ans):
        ...
        if ans in player.nans:
            ...
            await ctx.send(f'**`{ctx.author}`** try again?')

    @commands.command(name='aq')
    async def _view_ans(self, ctx):
        """get answer queue"""
        vc = ctx.voice_client
        ...
        player = self.get_player(ctx)
        if player.ans_que.empty():
            return await ctx.send('There are currently no more queued songs.')

        upcoming = list(enumerate(itertools.islice(player.ans_que._queue, 0, 10)))
        print(*upcoming, sep = '\n')
        #fmt = '\n'.join(i for i in upcoming)
        #embed = discord.Embed(title=f'Upcoming - Next {len(upcoming)}', description=fmt)
